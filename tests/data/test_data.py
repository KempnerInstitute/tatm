...
def test_json_corpus_data_load():
    for corpus in ["primary", "secondary"]:
        dataset = get_data(f"tests/data/json_corpus_data:{corpus}")
        item = 0
        for i in dataset:
            if item == 0:
                if corpus == "primary":
...
def test_json_corpus_full_data_load():
    dataset = get_data("tests/data/json_corpus_data")
    item = 0
    for i in dataset:
        if item == 0:
            assert i["text"] == "This is the first primary sentence."
